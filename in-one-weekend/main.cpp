...
int main(int argc, char* argv[]) {
    ...
    // read in number of threads to use if specified,
    // single-threaded if not specified
    long int num_threads = 1;
    if (argc > 1) {
        num_threads = strtol(argv[1], nullptr, 10);
        if (num_threads < 1) {
            num_threads = 1;
        }
    }

    // config for rendering execution
    scene_params sp;
    sp.image_height = image_height;
    sp.image_width = image_width;
    sp.samples_per_pixel = samples_per_pixel;
